from apiiif.resource_types import AnnotationPage, Canvas, Manifest, Collection
...
def test_add_multiple_images():
    ...
    assert (
        annotation.body.items[0] != annotation.body.items[1] != annotation.body.items[2]
    )


def test_add_annotation():
    annotation = get_annotation()
    annotation_page = AnnotationPage(id="http://example.org/iiif/book1/page/p1/1")
...
def test_add_canvas():
    manifest = Manifest(
        id="http://example.org/iiif/book1/manifest", label={"en": ["Book 1"]}
    )
    canvas = Canvas(
        id="http://example.org/iiif/book1/canvas/p1",
        label={"en": ["p. 1"]},
